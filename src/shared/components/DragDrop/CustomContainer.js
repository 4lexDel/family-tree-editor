...
export function CustomDragDrop({
  ...
  function handleDrop(e, type) {
    ...
    const allFilesValid = files.every((file) => {
      ...
      return formats.some((format) => words[words.length - 1].toUpperCase() === format.toUpperCase());
    });
    ...
    if (files && files.length) {
      const nFiles = files.map(async (file) => {
        ...
        return {
          name: removeLastSegment(file.name),
          data: fileData,
          type: file.type,
          size: file.size
        };
  ...
  function removeLastSegment(str) {
    ...
    // Si un point est trouvé, retourner la partie de la chaîne avant ce point
    if (lastIndex !== -1) {
      return str.substring(0, lastIndex);
    }

    // Si aucun point n'est trouvé, retourner la chaîne originale
    return str;
  }
  ...
  function showFile(title, text) {
    Swal.fire({
      title: title,
      html: `<pre style="text-align: start; height: 500px; overflow: auto;">${text}</pre>`,
    ...
    <>
      ...
      <div
        className={`${dragging
          ...
          } flex items-center justify-center mx-auto text-center border-2 rounded-md mt-4 py-5 pb-5`}
        ref={dropContainer}
      >
            ...
            <div key={index} className="w-full px-3 py-3.5 rounded-md bg-slate-200 space-y-3">
              <div className="flex justify-between">
                <div className="w-[70%] flex justify-start items-center space-x-2">
                  <div className="text-[#5E62FF] text-[37px] cursor-pointer" onClick={() => showFile("GEDCOM", file.data)}>
                    <FaRegFile />
                  </div>
